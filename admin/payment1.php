...
function fetch_pending_grns($vendor_id, $branch_id)
{
    ...
    $has_transportation = $grcols && isset($grcols['transportation']);
    $transport_col = $has_transportation ? ", gr.transportation" : "";

    if (table_exists('goods_receipts') && table_exists('purchase_orders')) {
        $rows = exeSql("SELECT gr.grn_id, gr.grn_number, gr.total_amount $transport_col
    ...
    } else if ($grcols && isset($grcols['branch_id'])) {
        $rows = exeSql("SELECT grn_id, grn_number, grn.total_amount $transport_col
    ...
    $paidMap = [];
    if (table_exists('vendor_grn_payments')) {
        $id_list = implode(',', array_map('intval', $ids));
        if ($id_list) {
            $paidRows = exeSql("
                SELECT grn_id, 
                       IFNULL(SUM(amount),0) + IFNULL(SUM(advance_used),0) + IFNULL(SUM(redemption_used),0) AS total_credit
                FROM vendor_grn_payments 
                WHERE grn_id IN ($id_list) 
                GROUP BY grn_id
            ");
            foreach ($paidRows ?: [] as $r) {
                $paidMap[(int)$r['grn_id']] = (float)$r['total_credit'];
            }
        }
    }

    $rows = enrich_and_filter_pending($rows, $paidMap, $rtMap, $has_transportation);

    return $rows ?: [];
}

function enrich_and_filter_pending(array $rows, array $paidMap, array $returnMap, bool $has_transportation)
{
    $out = [];
    foreach ($rows as $r) {
        $grn_id = (int)$r['grn_id'];
        ...
        $paid   = (float)($paidMap[$grn_id] ?? 0.0);

        $transport = $has_transportation ? (float)($r['transportation'] ?? 0.0) : 0.0;

        $total  = max(0.0, $gross + $transport - $rt);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!in_array($pay_for, ['vendor', 'employee', 'expenses', 'fixed'], true)) {
        header("Location: " . $_SERVER['REQUEST_URI'] . "?err=type");
        exit;
    }
    ...
    // FIXED: Get GRN splits from client-side calculation
    if ($payload['pay_for'] === 'vendor') {
        $client_splits = v('grn_splits', '');
        if ($client_splits) {
            $decoded = json_decode($client_splits, true);
            $payload['grn_splits'] = is_array($decoded) ? $decoded : [];
        } else {
            $payload['grn_splits'] = [];
        }
    }
...
LIMIT 1");
            ...
            header("Location: dashboard.php?msg=updated&rid=" . $existing_rid);
            exit;
        } else {
            header("Location: dashboard.php?msg=locked&rid=" . $existing_rid);
            exit;
        }
    }
    ...
    header("Location: dashboard.php?msg=submitted&rid=" . $newRid);
    exit;
}
...
<body>
    ...
    <a href="dashboard.php" class="btn-dashboard">⬅️ Requester Dashboard</a>
    <?php
    if (isset($_GET['err'])):
    ...
    <form method="post" id="paymentForm">
        ...
        <fieldset id="expenses_block" class="hide">
            ...
            <div class="row <?= ($def_purpose === '__other__' || $def_custom_purpose ? '' : 'hide') ?>" id="custom_total_wrap">
                <div><label>Total (new purpose)</label><input type="number" step="0.01" name="custom_total" id="custom_total" value="<?= $def_custom_total ? (float)$def_custom_total : '' ?>"></div>
        ...
        <fieldset id="amounts_block" class="hide">
            ...
            <div class="totals">
                <div><label>Total Amount</label><input type="number" step="0.01" id="total_amount" name="total_amount" readonly value="<?= $def_total ? (float)$def_total : '' ?>"></div>
                ...
                <div><label>Pay Now</label><input type="number" step="0.01" name="pay_now" id="pay_now" min="1" value="" required></div>
            </div>
            <div id="split_preview" class="hide" style="margin-top: 15px; padding: 10px; background: #e7f3ff; border: 1px solid #b3d9ff; border-radius: 4px;">
                <strong>Payment Split Preview:</strong>
                <div id="split_preview_content" style="margin-top: 8px; font-size: 0.9em;"></div>
            </div>
        </fieldset>
    ...
    <script>
        ...
        const defPayNow = <?= (float)$def_pay_now ?>;
        ...
        function toggleSections() {
            ...
            if (payFor.value === 'employee') {
                show(empBl);
                loadEmp();
            }
            if (payFor.value === 'expenses') {
                show(expBl);
                loadExpenseSummary();
            }
        ...
        function setSubmitEnabled(on, reason = '') {
            if (on) {
        ...
        async function liveDupCheck() {
            ...
            if (pf === 'vendor') {
                // OPTIMIZED: Check splits instead of just GRN IDs
                const splits = calculateGrnSplits();
                if (splits.length === 0) {
                    clearDup();
                    return;
                }
                // Only check GRNs that will actually receive payment
                const grnIdsWithPayment = splits.map(s => s.grn_id).join(',');
                if (grnIdsWithPayment) qs.set('grn_ids', grnIdsWithPayment);
            } else if (pf === 'employee') {
        ...
        /* ------- GRN Hidden Inputs & Splits Calculation ------- */
        function syncGrnHiddenInputs() {
            hiddenGrnInputs.innerHTML = '';
            // Only send GRN IDs that are actually receiving payment
            const splits = calculateGrnSplits();
            splits.forEach(split => {
                const input = document.createElement('input');
                input.type = 'hidden';
                input.name = 'grn_ids[]';
                input.value = split.grn_id;
                hiddenGrnInputs.appendChild(input);
            });
        }

        function updateSplitPreview(splits) {
            ...
            html += '<tr style="border-bottom: 2px solid #0056b3; font-weight: 600; background: #cfe2ff;">';
            html += '<td style="padding: 8px;">GRN Number</td>';
            html += '<td style="padding: 8px; text-align: right;">Current Balance</td>';
            html += '<td style="padding: 8px; text-align: right;">Paying Now</td>';
            html += '<td style="padding: 8px; text-align: right;">After Payment</td>';
            html += '<td style="padding: 8px; text-align: center;">Status</td>';
            html += '</tr>';

            splits.forEach(split => {
                const afterPayment = split.balance - split.amount;
                const isComplete = afterPayment < 0.01;
                const statusColor = isComplete ? '#0f5132' : '#856404';
                const statusBg = isComplete ? '#d1e7dd' : '#fff3cd';
                const statusText = isComplete ? '✓ Complete' : '⊙ Partial';

                html += '<tr style="border-bottom: 1px solid #b3d9ff;">';
                html += `<td style="padding: 8px;"><strong>${split.grn_number}</strong></td>`;
                html += `<td style="padding: 8px; text-align: right;">₹${split.balance.toFixed(2)}</td>`;
                html += `<td style="padding: 8px; text-align: right; color: #007bff; font-weight: 700; font-size: 1.05em;">₹${split.amount.toFixed(2)}</td>`;
                html += `<td style="padding: 8px; text-align: right; ${isComplete ? 'color: #0f5132; font-weight: 600;' : ''}">${isComplete ? '₹0.00' : '₹' + afterPayment.toFixed(2)}</td>`;
                html += `<td style="padding: 8px; text-align: center;"><span style="display: inline-block; padding: 4px 10px; border-radius: 4px; font-size: 0.85em; font-weight: 600; background: ${statusBg}; color: ${statusColor};">${statusText}</span></td>`;
                html += '</tr>';
            });
        ...
        function calculateGrnSplits() {
            const checked = Array.from(document.querySelectorAll('.grn-cb:checked'));
            const payNowAmount = parseFloat(payNow.value || 0);

            if (checked.length === 0 || payNowAmount <= 0) {
                updateSplitPreview([]);
                return [];
            }

            // SEQUENTIAL PAYMENT: Pay GRNs line by line until payment runs out
            let remaining = payNowAmount;
            const splits = [];
            ...
            for (const cb of checked) {
                ...
                const payThis = Math.min(balance, remaining);

                splits.push({
                    grn_id: parseInt(cb.value),
                    grn_number: cb.closest('.grn-item').querySelector('strong').textContent,
                    balance: balance,
                    amount: Math.round(payThis * 100) / 100
                });

                remaining -= payThis;
            }

            updateSplitPreview(splits);
            return splits;
        }

        function syncGrnSplits() {
            const existingSplits = document.querySelector('input[name="grn_splits"]');
            if (existingSplits) existingSplits.remove();

            const splits = calculateGrnSplits();
            if (splits.length > 0) {
                const input = document.createElement('input');
                input.type = 'hidden';
                input.name = 'grn_splits';
                const cleanSplits = splits.map(s => ({
                    grn_id: s.grn_id,
                    amount: s.amount
                }));
                input.value = JSON.stringify(cleanSplits);
                hiddenGrnInputs.appendChild(input);
            }
        }
        ...
        function renderGRNCheckboxes(list) {
            ...
            list.forEach(g => {
                ...
                cb.className = 'grn-cb';
                cb.value = g.grn_id;
                cb.dataset.gross = g.gross_amount;
                cb.dataset.transport = g.transport;
                cb.dataset.returns = g.returns;
                ...
                const gross = Number(g.gross_amount).toFixed(2);
                const transport = Number(g.transport).toFixed(2);
                const returns = Number(g.returns).toFixed(2);
                const total = Number(g.total_amount).toFixed(2);
                const paid = Number(g.paid).toFixed(2);
                const balance = Number(g.balance).toFixed(2);

                meta.innerHTML = `Gross: ₹${gross} | Transport: ₹${transport} | Returns: ₹${returns}<br>Total Due: ₹${total} | Paid/Credited: ₹${paid} | **Bal: ₹${balance}**`;
        ...
        function recalcFromChecked() {
            ...
            cbs.forEach(cb => {
                ...
                paidSum += parseFloat(cb.dataset.paid || 0);
                balanceSum += parseFloat(cb.dataset.balance || 0);
            });
            setVal(totalAmount, totalSum.toFixed(2));
            setVal(paidSoFar, paidSum.toFixed(2));
            setVal(balance, balanceSum.toFixed(2));
        ...
        async function loadExpenseSummary() {
            ...
            if (purpose === '__other__') {
                ...
                const customT = parseFloat(customTotal.value || '0');

                setVal(payNow, customT > 0 ? '' : '0');

                setVal(totalAmount, customT > 0 ? customT.toFixed(2) : '');
                showAmounts();
                recomputeBalance();
        ...
        async function loadFixedList() {
            try {
                ...
                list.forEach(r => {
                    ...
                    if (String(r.id) === String(defFixed)) opt.selected = true;
                    frag.appendChild(opt);
                });
        ...
        async function loadFixedOne() {
            ...
            try {
                ...
                const total = Number(r.amount || 0),
                    paid = Number(r.balance_paid || 0),
                    bal = Number(r.remaining_balance || 0);
                setVal(totalAmount, total.toFixed(2));
                setVal(paidSoFar, paid.toFixed(2));
                setVal(balance, bal.toFixed(2));

                setVal(payNow, bal > 0 ? '' : '0');
                showAmounts();
            } catch (e) {
                console.error(e);
                hideAmounts();
        ...
        function recomputeBalance() {
            ...
            if (payNow.value === '' && t - p > 0) {
                setVal(balance, (t - p).toFixed(2));
            }
        }
        payNow.addEventListener('input', () => {
            recomputeBalance();
            if (payFor.value === 'vendor') {
                syncGrnSplits();
            }
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            if (editRid > 0 && defPayNow > 0) {
                setVal(payNow, defPayNow.toFixed(2));
            }
