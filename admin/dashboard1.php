...
// *** MODIFIED CONSOLIDATED TABLE LOGIC FOR FIXED ***
					elseif ($type === 'fixed') {
						$purpose = (string)($payload['purpose']??'');
						// Replicate the fixed_id fallback logic from render_fixed_table_single to reliably get the name
						...
						// ONLY display the expense type, removing the (FE ID: X) part
						$details = 'Fixed Expense: ' . h(ucfirst($purpose));
					}
// *** END MODIFIED CONSOLIDATED TABLE LOGIC ***
 ...
 * FIX: Safely fetches expense_type from the fixed_expenses table if not in the request payload.
 */
...
<div class="card shadow-sm border-0 mb-3"><div class="card-body p-2">
	<div class="table-responsive">
		<table class="table table-striped table-hover align-middle small">
			...
			<tbody>
			<?php if(!$rows): ?>
				<tr><td colspan="9" class="text-muted"><?=h($emptyMsg)?></td></tr>
			<?php else: foreach($rows as $r):
				$p = json_decode($r['payload_json']??'{}',true)?:[];
				$purpose = (string)($p['purpose']??'');
				// Fallback: If purpose is empty in the payload, try to fetch it from the fixed_expenses table
				if (empty($purpose) || $purpose === '_other_') {
					$fixedId = (int)($p['fixed_id'] ?? 0);
					if ($fixedId > 0 && table_exists('fixed_expenses')) {
						$feRow = exeSql("SELECT expense_type FROM fixed_expenses WHERE id=$fixedId LIMIT 1");
						if ($feRow) {
							$purpose = (string)($feRow[0]['expense_type'] ?? 'N/A');
...
<header class="page-title">
	...
	<div class="d-flex gap-2">
	    <a href="/kem/finance/" class="btn btn-secondary">Finance Dashboard</a>
