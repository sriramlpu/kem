...
<style>
  ...
  /* --- HIDE DISCOUNT FROM UI but keep it in DOM --- */
  .discount-col { display: none !important; } /* hides discount th/td cells */
  #discountAmount { display: none !important; } /* hides top-level discount input (now hidden) */
</style>
        ...
        <div class="card-body">
          <form id="poForm">
            <div class="row g-4">
              ...
              <!-- Indent Number (hidden field for API) -->
              <div class="col-md-3">
                <div class="form-floating">
                 <input type="hidden" id="indentHidden" name="indent_number" value="0">
                  <select id="indentNumber" name="indentNumber" class="form-select select2" disabled>
              ...
              <div class="col-md-3">
                <div class="form-floating">
                  <input type="number" step="any" id="totalAmount" name="total_amount" class="form-control" placeholder="Total Amount" readonly>
                  <label for="totalAmount">Total Amount(₹)</label>
                </div>
              </div>

              <!-- Discount Amount (HIDDEN - still submitted as 0) -->
                ...
                <table class="table table-bordered" id="itemsTable">
                  <thead>
                    <tr>
                      ...
                      <th>Quantity</th>
                      <th>Unit Price(₹)</th>
                      <th class="discount-col">Discount Amt(₹)</th> <!-- hidden -->
 ...
 $(document).ready(function() {
  ...
  function populateAddressDropdowns() {
    ...
    if (allBranchAddresses.length === 0) {
      ...
      billingSelect.append(defaultOption);
        ...
        // console.log(allBranchAddresses)
      allBranchAddresses.forEach(function(branch) {
        ...
        const option = `<option value="${branch.branch_id}" data-branch="${branch.branch_name}">${full}</option>`;
  ...
  $('#itemCategory').on('change', function() {
    ...
    $.ajax({
      url: './api/items_api.php',
      method: 'POST',
      data: { category_id: catId, action: 'getSubCategories' },
      dataType: 'json',
      success: function(res) {
        if (res.status === 'success') {
  ...
  $('#itemSubCategory').on('change', function() {
    ...
    $.ajax({
      url: './api/items_api.php',
      method: 'POST',
      data: { subcategory_id: subId, action: 'getItemsBySubCategory' },
      dataType: 'json',
      success: function(res) {
        // console.log(res);
        if (res.status === 'success') {
  ...
  function addItemRow(item = {}) {
    ...
    const row = $(` 
      <tr data-item-id="${item.item_id}">
        ...
        <td><input type="number" step="any" name="items[${itemCount}][quantity]" class="form-control item-quantity" value="${item.quantity}" required></td>
        <td><input type="number" step="any" name="items[${itemCount}][unit_price]" class="form-control item-unit-price" value="0" required></td>

        <!-- Discount cell kept in DOM but hidden via .discount-col -->
        <td class="discount-col">
          <input type="hidden" step="any" name="items[${itemCount}][discount_amount]" class="form-control item-discount-amt" value="0">
        </td>

        <td><input type="number" step="any" name="items[${itemCount}][tax_percentage]" class="form-control item-tax-pct" value="${item.tax_percentage}"></td>
        <td><input type="number" step="any" name="items[${itemCount}][tax_amount]" class="form-control item-tax-amt" value="0" readonly></td>
        <td><input type="number" step="any" name="items[${itemCount}][subjective_amount]" class="form-control item-line-total" value="0" readonly></td>
        <td>
    ...
    vendorDropdown.on('change', function() {
      ...
      const itemId = currentRow.data('item-id');
      const unitPriceInput = currentRow.find('.item-unit-price');

      if (selectedVendorId && itemId) {
        // Fetch last purchase price
        $.ajax({
          url: './api/purchase_order_api.php',
          method: 'POST',
          data: {
            action: 'getLastPurchasePrice',
            item_id: itemId,
        ...
        // Auto-fill unit price and tax percentage
        unitPriceInput.val(parseFloat(res.data.unit_price).toFixed(4));
        calculateLineTotal(currentRow);

        showAlert(`Unit price auto-filled from last purchase (${res.data.po_date})`, 'info');
    } else {
        // No last purchase price found → set 0
        unitPriceInput.val('0.0000');
        calculateLineTotal(currentRow);
    }
          },
          error: function() {
            console.log('Could not fetch last purchase price');
          }
        });
      }
    });
...
row.find('.btn-split').on('click', function() {
  const currentRow = $(this).closest('tr');
  const itemId = currentRow.data('item-id');
  const itemName = currentRow.find('td:first').text();
  ...
  // Disable already used vendors
  newRow.find('.item-vendor option').each(function() {
    if (usedVendors.includes($(this).val())) $(this).prop('disabled', true);
  });

  // Insert after current row
  currentRow.after(newRow);
  disableUsedVendors(itemId);

  // Re-initialize Select2 after disabling
  newRow.find('.item-vendor').select2({
    width: '100%',
    placeholder: 'Select Vendor',
    allowClear: true
  });
  ...
  // ============ CALCULATION LOGIC (ADJUSTED: discount ignored, GST included in item total) ============
  ...
  // Update tax amount based on tax percentage (discount ignored here)
  function updateTaxAmount(row) {
    ...
    // Discount is intentionally ignored in calculations (kept as 0)
    const base = (q * up);
    const taxAmt = (base * taxRate) / 100;
    row.find('.item-tax-amt').val(taxAmt.toFixed(2));
  }

  // Calculate line total for a specific row (GST included)
  function calculateLineTotal(row) {
    ...
    // Discount ignored
    const lineTotal = Math.max(0, (q * up)) + Math.max(0, tam);
    row.find('.item-line-total').val(lineTotal.toFixed(2));
    
    calculateTotal();
  }

  // Calculate overall total (discount ignored)
  function calculateTotal(opts) {
    ...
    // Grand total includes transportation as well
    const grand = total + transportation;
    $('#totalAmount').val(grand.toFixed(2));

    // Ensure discount hidden field remains zero
  ...
  // Keep handler for tax pct input
  ...
  // transportation affects total
  $('#transportation').on('input change blur', function() {
  ...
  $('#poForm').on('submit', function(e) {
    ...
    // Ensure all discount inputs are set to 0 before sending (safety)
    $('#itemsTableBody tr').each(function() {
      $(this).find('.item-discount-amt').val('0');
    });
    $('#discountAmount').val('0');
    ...
    $.ajax({
      ...
      complete: function() {
        ...
        submitBtn.prop('disabled', false).text('Create PO');
      }
    });
  });

  function disableUsedVendors(itemId) {
    const usedVendors = [];
    $(`#itemsTableBody tr[data-item-id="${itemId}"]`).each(function() {
      const v = $(this).find('.item-vendor').val();
      if (v) usedVendors.push(v);
    ...
    $(`#itemsTableBody tr[data-item-id="${itemId}"] .item-vendor`).each(function() {
      $(this).find('option').each(function() {
        if ($(this).val() && usedVendors.includes($(this).val()) && $(this).val() !== $(this).closest('select').val()) {
...
// keep indentHidden updated
$('#indentNumber').on('change', function() {
  $('#indentHidden').val($(this).val());
});
