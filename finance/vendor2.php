<?php
/**
 * vendor.php - Vendor Summary & Ledger
 * UPDATES (ONLY):
 * 1) Branch filter logic (Code-1): when Branch is selected, vendor list shows ONLY vendors having GRNs in that branch.
 * 2) Ledger: show GRN ONLY ONE TIME (NO separate PAYMENT rows). Payment is shown in the same GRN row (Out = grn.paid_amount).
 */
...
// ✅ UPDATED BRANCH FILTER LOGIC (ONLY CHANGE):
// When branch is selected, show ONLY vendors who have GRNs in that branch (i.e., stats exists)
if ($branch_id > 0) {
...
if (!empty($vendors)) {
    ...
    if (!empty($ids)) {
        ...
        // ✅ LEDGER (ONLY GRN ROWS) — payment shown in same row via grn.paid_amount
        $q1 = "
            SELECT
                grn.vendor_id, 
                'GRN' AS type, 
            ...
            FROM goods_receipts grn
            LEFT JOIN (
                SELECT grnrt.grn_id, SUM(gri.total_amount) AS rtn
                FROM goods_return_items gri
        ...
        // Sorting: Pending first, then date
        $full_query = "
